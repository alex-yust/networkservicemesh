...
func cleanupImages(cmd *imageCleanupCmd) {
	tags, err := listImageTags(fmt.Sprintf("%s/%s", cmd.repo, cmd.images[0]), cmd.imageMinAge, cmd.count)
	if err != nil {
		logrus.Errorf("Failed to get tag list: %v", err)
		return
	}
...
func listImageTags(repo string, minAge time.Duration, maxCount int) ([]string, error) {
	var tags []string
	nextUrl := fmt.Sprintf("https://hub.docker.com/v2/repositories/%s/tags", repo)
	for ; nextUrl != "null"; {
		...
		for _, jt := range tagPage.Results {
			if lu, err := time.Parse(time.RFC3339, jt.LastUpdated); err == nil && time.Since(lu) < minAge {
				continue
			}
			logrus.Infof("tag: %v", jt)
			//tags = append(tags, jt.Name)
			if len(tags) >= maxCount {
				return tags, nil
			}
		}
		nextUrl = tagPage.Next
